...
export const ReducerData = (state = initialstate, action) => {
    switch (action.type) {
        ...
        case "ADD_MEN_DATA":
            ...
            localStorage.setItem('MEN', JSON.stringify(getData));
            return {
                ...state,
                men: getData
            }
        case "DELETE_MEN_DATA":
            let DeleteData = JSON.parse(localStorage.getItem('MEN')) || [];
            let DelData = DeleteData.filter((v => v.id != action.payload))
            localStorage.setItem('MEN', JSON.stringify(DelData));
            return {
                ...state,
                men: DelData
            }
        case "GET_ONE_MEN_DATA":
            let OneRecord = JSON.parse(localStorage.getItem('MEN')) || [];
        ...
        case "UPDATE_MEN_DATA":
            let UpdateData = JSON.parse(localStorage.getItem('MEN')) || [];
            ...
            localStorage.setItem('MEN', JSON.stringify(uprecord));
            return {
                ...state,
                MenData: null,
                men: uprecord
        ...
        case "FILTER_MEN_DATA":
            let allMen = JSON.parse(localStorage.getItem("MEN")) || [];
