...
//                                                 <div className='mens-card' onClick={() => navigate(`/view/${v.id}`)}>
...
import { useDispatch, useSelector } from 'react-redux';
import { DeleteKidsDataAsync, filterData, GetAllKidsAsync } from '../Services/Action/Action';
import { useNavigate } from 'react-router-dom'; // <- important: react-router-dom
import { useEffect, useState } from 'react';
import { IoChevronDownSharp } from "react-icons/io5";
import { CiHeart } from "react-icons/ci";
import Loader from '../Loader/Loader';
...
const Kids = () => {
  ...
  const handleDelete = (id, e) => {
    // e might be undefined if called programmatically; defensive:
    if (e) {
      e.stopPropagation();
      // stopImmediatePropagation for rare cases
            ...
            <div className="col-5 col-lg-3">
              <div className="sidebar p-3 border rounded">
                ...
                <div className="price d-flex justify-content-between mb-3">
                  <div>
                    <span style={{ color: "grey" }}>Min.Amount</span>
                    <input type="number" min={100} placeholder="100" style={{ width: "100px", border: "1px solid grey", padding: "5px", borderRadius: "5px" }} />
                  </div>
                  <div>
                    <span style={{ color: "grey" }}>Max.Amount</span>
                    <input type="number" max={5000} placeholder="5000" style={{ width: "100px", border: "1px solid grey", padding: "5px", borderRadius: "5px" }} />
                  </div>
                </div>

                <div className="mb-3">
                  <div className="d-flex justify-content-between align-items-center" style={{ cursor: "pointer" }} onClick={() => setShowCategory(!showCategory)}>
                    <h6 className="mb-0">Category Type</h6>
                    <IoChevronDownSharp
                      style={{
                        transform: showCategory ? "rotate(180deg)" : "rotate(0deg)",
                ...
                <div>
                  <div className="d-flex justify-content-between align-items-center" style={{ cursor: "pointer" }} onClick={() => setShowBrand(!showBrand)}>
                    <h6 className="mb-0">Brand</h6>
                    <IoChevronDownSharp
                      style={{
                        transform: showBrand ? "rotate(180deg)" : "rotate(0deg)",
                    ...
                    {men.map((v) => (
                      <div key={v.id} className='col-12 col-lg-4'>
                        <div className='mens-card'>
                          {/* only clicking this image area will open view */}
                          <div
